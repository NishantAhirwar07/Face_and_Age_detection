...
class ModelLoadError(RuntimeError):
    """Raised when a model file exists on disk but OpenCV can't parse it
    (e.g. it's a leftover partial/corrupt download). The offending file is
    deleted so the next call to ensure_models() will re-download it.
    """
...
def ensure_models(progress_callback: Optional[Callable[[str], None]] = None) -> None:
    ...
    Downloads are written to a temporary path first and only moved into
    place once they complete successfully, so an interrupted download can
    never masquerade as a valid, already-downloaded file on the next run.
    """
    ...
    for filename, url in MODEL_FILES.items():
        ...
        try:
            ...
            tmp_dest.replace(dest)  # atomic on POSIX and Windows
        except Exception as exc:  # noqa: BLE001 - surfaced to the caller
            tmp_dest.unlink(missing_ok=True)
            raise ModelDownloadError(f"Could not download {filename}: {exc}") from exc


def _discard_model_files() -> None:
    """Delete all model files so the next ensure_models() call re-downloads
    everything. Used as a recovery step after a ModelLoadError."""
...
class FaceAgeDetector:
    ...
    def load(self, progress_callback: Optional[Callable[[str], None]] = None) -> "FaceAgeDetector":
        ...
        try:
            ...
            # Almost always means a model file on disk is truncated/corrupt
            # (e.g. an interrupted download). Discard everything so the next
            # attempt re-downloads clean copies instead of failing forever.
            _discard_model_files()
            raise ModelLoadError(
                "A model file appears to be corrupted or incomplete and has "
                f"been removed. Please retry. ({exc})"
            ) from exc
        return self
