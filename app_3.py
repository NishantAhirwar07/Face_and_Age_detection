...
from detector import FaceAgeDetector, ModelDownloadError, ModelLoadError, ScanResult
...
def load_detector_with_feedback() -> FaceAgeDetector | None:
    try:
        ...
        st.error(
            "One of Chronoscope's model files was corrupted (likely an "
            f"interrupted download). Details: {exc}"
        )
